...
SBD's flat file (codename "SBD", proof-of-concept round 3), joined into this
repo's column shape (originally blocked on the `data-exchange-demo-poc3-inbound`
UC grant as of 2026-08-25, see #central-it-support; the grant has since landed
and the file is present locally as `sbd_poc3.parquet`). Used as a
realistic-scale/realistic-noise alternative to
`generate_standard_simulation_rows(...)`'s synthetic data. Not this repo's job
to redo that join or fix source data quality (unit mixing in lead_time_days,
the 4-vocabulary product-id crosswalk, etc.) — that's upstream, on whoever
hands us the file.

**Not committed to git** (see `.gitignore`: `fixtures/**/*.csv` and
`fixtures/**/*.parquet` — never commit real customer data); present locally
only. If this file is missing, `tests/test_notebooks.py`'s sbd_poc3 fixture
