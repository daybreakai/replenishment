...
def _fetch(catalog: str, schema: str, profile: str) -> tuple[list[dict], dict[str, float], dict[str, int]]:
    shipments = run_query(f"SELECT product_id, actual_ship_date, shipped_qty FROM {catalog}.{schema}.t_outbound_shipment", profile)
    product_rows = run_query(f"SELECT id AS product_id, unit_cost FROM {catalog}.{schema}.t_product", profile)
    lead_time_rows = run_query(f"SELECT product_id, lead_time_days FROM {catalog}.{schema}.t_vendor_lead_time", profile)

    unit_cost_by_id = {r["product_id"]: float(r["unit_cost"]) for r in product_rows if r["unit_cost"] is not None}
    lead_time_by_id = {r["product_id"]: int(float(r["lead_time_days"])) for r in lead_time_rows if r["lead_time_days"] is not None}
    return shipments, unit_cost_by_id, lead_time_by_id
...
def derive_rows(
    ...
    shipments, unit_cost_by_id, lead_time_by_id = _fetch(catalog, schema, profile)
...
def main() -> None:
    ...
    try:
        rows = derive_rows(
            ...
            order_cost=args.order_cost, forecast_window=args.forecast_window,
        )
        if not rows:
