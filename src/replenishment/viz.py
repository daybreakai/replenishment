...
from replenishment.io_ import (
    ...
    _aggregate_lead_time as aggregate_lead_time,
    _aggregate_series as aggregate_series,
...
# ponytail: service_level_multiplier ported here (scoped to this module only)
# from janrth's service_levels.py -- it's the one small pure helper viz.py
# needs from that module, and nothing else in this repo needs it yet.
...
def plot_replenishment_decisions(
    ...
    *,
    ...
    if "missed_sales" in decisions_df.columns:
        ...
        # ponytail: bugfix vs. janrth's original -- lost_sales carries
        # series.index (RangeIndex) while the "if" branch above produces a
        # ds-indexed Series; later code (.divide against a ds-indexed
        # cumulative_demand) assumed ds-indexing in both branches, so the
        # RangeIndex here silently broke alignment (misaligned union
        # instead of an error, in real usage) whenever missed_sales wasn't
        # present. Reindex to ds to match the other branch's convention.
        loss_series = pd.Series(lost_sales.fillna(0).values, index=series["ds"])
